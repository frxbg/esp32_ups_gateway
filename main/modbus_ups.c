...
// Global UPS data instance
UpsData current_ups_data = {0};
...
esp_err_t modbus_ups_slave_init(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Modbus controller initialization failed: %s", esp_err_to_name(err));
        return err;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set register descriptor: %s", esp_err_to_name(err));
        mbc_slave_destroy();
        return err;
    }
    ...
    if (config.parity == 1) parity_str = "Odd";
    else if (config.parity == 2) parity_str = "Even";
    ...
    ESP_LOGI(TAG, "  TX Pin: %d, RX Pin: %d, RTS Pin: %d", 
             MODBUS_UART_TXD_PIN, MODBUS_UART_RXD_PIN, MODBUS_UART_RTS_PIN);
    ESP_LOGI(TAG, "  Slave Address: %d", config.slave_addr);
    ESP_LOGI(TAG, "  Holding Registers: %d (offset %d)", MODBUS_REG_COUNT, MODBUS_REG_START);
...
void modbus_ups_update_registers(const UpsData *data)
{
    ...
    holding_reg[5] = 0;
    if (data->status_ac_present) holding_reg[5] |= (1 << 0);
    if (data->status_charging) holding_reg[5] |= (1 << 1);
    if (data->status_discharging) holding_reg[5] |= (1 << 2);
    if (data->status_overload) holding_reg[5] |= (1 << 3);
    if (data->status_below_capacity_limit) holding_reg[5] |= (1 << 4);
    if (data->status_fully_charged) holding_reg[5] |= (1 << 5);
    if (data->status_boost) holding_reg[5] |= (1 << 6);
    if (data->status_time_limit_expired) holding_reg[5] |= (1 << 7);
    ...
    holding_reg[10] = data->battery_voltage_dV > 30 ? data->battery_voltage_dV * 0.941f : (data->battery_voltage_dV * 0.941f) + data->battery_voltage_nominal_dV;                      // 11: Battery voltage (dV)
    holding_reg[11] = data->battery_voltage_nominal_dV;              // 12: Nominal voltage (dV)
